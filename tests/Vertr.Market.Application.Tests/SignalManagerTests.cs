...
public class SignalManagerTests
{
    ...
    [Fact]
    public async Task ConcurrentSnapshotAndWrite_SnapshotConsistent()
    {
        const int capacity = 64;
        ...
        for (var i = 0; i < capacity; i++)
        {
            manager.WriteSignal(i, i * 1.0);
        }

        var capturedData = new double[capacity];
        var capturedLock = new object();
        var snapshotTaken = new ManualResetEventSlim(false);
        ...
        for (var i = 0; i < capacity; i++)
        {
            Assert.True(double.IsFinite(capturedData[i]) || capturedData[i] == 0.0, $"Value at index {i} is not finite: {capturedData[i]}");
        }
    }
    ...
    public void SnapshotData_CopyTo_DestinationsSpan()
    {
        ...
        for (var i = 0; i < capacity; i++)
        {
            Assert.Equal(i * 2.0, destination[i]);
        }
    }
